...
export function initPageCopyDropdown() {
  ...
  const aiButtons = dropdown.querySelectorAll('[data-action^="open-"], [data-action="connect-cursor"]');
  ...
  function setButtonState(button, state, text, icon) {
    button.classList.remove('page-copy-dropdown__item--loading', 'page-copy-dropdown__item--success', 'page-copy-dropdown__item--error');
    button.classList.add(`page-copy-dropdown__item--${state}`);
    
    const titleElement = button.querySelector('.page-copy-dropdown__item-title');
    if (titleElement) {
  ...
  function buildMarkdownUrl() {
    ...
    return currentPath.endsWith('/') ? currentPath + 'index.md' : currentPath + '.md';
  }
  ...
  function buildAIServiceUrl(action) {
    ...
    switch (action) {
      case 'open-chatgpt':
        return buildServiceUrl('chatgpt', `Read and analyze this Buildkite documentation page so I can ask you questions about it: ${markdownUrl}`);
      case 'open-claude':
        return buildServiceUrl('claude', `Read and analyze this Buildkite documentation page so I can ask you questions about it: ${markdownUrl}`);
      case 'connect-cursor':
  ...
  function buildProductionMarkdownUrl() {
    ...
    const productionUrl = currentUrl.replace(/https?:\/\/localhost:\d+/, PRODUCTION_BASE_URL);
