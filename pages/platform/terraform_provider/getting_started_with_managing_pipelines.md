...
> By default, when you create a pipeline through the Buildkite interface, the repository webhook is created automatically for repositories connected through the GitHub App. The Terraform provider does not create the repository webhook for you. Add a `buildkite_pipeline_webhook` resource for each pipeline. Otherwise, pushes to the repository won't trigger builds.
