...
> SSO capabilities are only available to Buildkite customers on the [Pro or Enterprise](https://buildkite.com/pricing) plan. Which providers you can configure depends on your plan. See [Supported providers](#supported-providers) for the plan each provider requires.
...
Buildkite supports the following SSO providers, on the plans listed against each one:
...
Yes, organization admins can select whether a user is 'required' to use SSO or whether it is 'optional'. You can find this setting in the [organization's user settings](https://buildkite.com/organizations/~/users).
