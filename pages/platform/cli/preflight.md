...
Preflight works well with [promise job failure](/docs/pipelines/configure/promise-job-failure). If a running job declares a promised hard failure, Buildkite Pipelines can move the build to `failing` before the job exits, and Preflight can start reporting the failure earlier while the job continues uploading logs and test results. Reading promised failures from running jobs requires Buildkite CLI version `BUILDKITE_CLI_VERSION` or later.
