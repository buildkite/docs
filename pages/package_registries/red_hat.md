...
The following [Buildkite CLI](/docs/platform/cli) command can also be used to publish an RPM package to your Red Hat source registry from your local environment, once it has been [installed](/docs/platform/cli/installation) and [configured with an appropriate (registry write) token](/docs/platform/cli/configuration):
