...
Once your Java package registry has been [created](/docs/packages/manage-registries#create-a-registry), you can publish/upload packages (generated from your application's build) to this registry by configuring your `~/.m2/settings.xml` and application's relevant `pom.xml` files with the Maven XML snippets presented on your Java package registry's details page.
...
- `~/.m2/settings.xml`: the ID for your specific Java package registry in Buildkite and API write token (generated by Buildkite Packages) required to publish the package to this registry
- `pom.xml`: the reference to the required Maven plugin (to upload the package), along with the ID and URL for your specific Java (Maven) package registry in Buildkite.
...
1. Copy the following XML snippet, and paste it to modify your `~/.m2/settings.xml` file accordingly:
          ...
          <configuration>
            <httpHeaders>
              <property>
                ...
                <value>Bearer package-registry-write-token</value>
              </property>
            </httpHeaders>
          </configuration>
        </server>
    ...
    where:
    * `package-registry-write-token` is the Buildkite Packages-generated API token required to publish/upload packages to your Java package registry.
...
1. Copy the following XML snippet, and paste it to modify your `pom.xml` configuration file accordingly:
    ...
    where:
    * `org-slug-registry-name` is the ID of your Java package registry (above).
