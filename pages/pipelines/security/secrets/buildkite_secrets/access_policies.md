...
Policies are defined as a list of policy rules in YAML. Each _policy rule_ (beginning with a `-`) specifies one or more _claims_ that must be met for a build to access the Buildkite secret.

Each claim must specify one or more conditions. Conditions can be a single item or lists of strings. When a list of strings is provided, at least one value must match for the claim to match.
