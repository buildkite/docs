...
The primary recommendation is to secure these communications with [OIDC](/docs/pipelines/security/oidc), since OIDC tokens issued by Buildkite hosted agents, obtained through the [`buildkite-agent oidc` command](/docs/agent/v3/cli/reference/oidc), can be used to verify that network communications originate from those agents, which in turn are associated with a specific Buildkite organization, pipeline, or metadata associated with a pipeline's job. Using OIDC tokens to secure these communications means that communication can be done securely over the public internet with HTTPS, without the need for VPNs.
