...
To propagate the trace all the way through to agent-emitted spans, also enable `--tracing-propagate-traceparent` on your agents. See [Propagating traces to Buildkite agents](#open-telemetry-tracing-from-buildkite-agent-propagating-traces-to-buildkite-agents).
