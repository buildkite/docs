...
| `buildkite.job.concurrency_wait_time_ms` | `buildkite.job` (concurrency-group jobs only, omitted when platform-limited)   | Time in milliseconds the job spent in its first concurrency-group wait (zero if the job was immediately promoted, omitted if platform-limit wait cannot be separated) |
