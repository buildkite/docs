...
>🚧 <code>concurrency_group</code> and <code>block</code>/<code>input</code> steps
> Note that [block steps](/docs/pipelines/block-step) (including [input steps](/docs/pipelines/input-step)) cause the steps to be uploaded and scheduled at the same time, which breaks concurrency groups: it blocks jobs being added to the concurrency group, but does not adjust ordering once unblocked. The concurrency won't be added to the queue until the block step is unblocked, and once it does, the timestamp will be from the pipeline upload step.
