...
> Job log archiving is only available on an [Enterprise](https://buildkite.com/pricing) plan.
...
> 🚧 AWS S3 only
> Job log archiving currently supports Amazon S3 buckets in the `us-east-1` region only. Google Cloud Storage and Azure Blob Storage are not supported.
