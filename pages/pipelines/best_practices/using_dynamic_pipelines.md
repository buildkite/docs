...
**You want to combine file-change and branch conditions.** `if_changed` skips steps based on file changes and `if:` filters on branch, source, or other build attributes, but they can't be combined with OR logic in a single step definition. If you need something like `if: build.branch == "main" OR if_changed: "deploy/**"`, a dynamic upload can evaluate both conditions and generate the right steps. See [Combining file-change and branch conditions with if_changed](/docs/pipelines/configure/dynamic-pipelines/dynamic-pipelines-guide#combining-file-change-and-branch-conditions-with-if_changed).
