...
**Retry on different infrastructure for resource-related failures.** For out-of-memory, disk-exhaustion, or spot-preemption failures, a [`pre-exit` hook](/docs/agent/hooks) can use `pipeline upload` to add a new step targeting a bigger or more stable agent queue. See [Retrying on different infrastructure](#retrying-on-different-infrastructure) for the implementation.
