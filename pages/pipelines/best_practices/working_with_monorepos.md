...
- **[Retry configuration](/docs/pipelines/configure/retry):** For transient failures like spot instance preemption, network timeouts, or flaky dependencies. Set `automatic` retry with `limit: 2` and your generator applies it to every step it produces, so you don't need to maintain retry policies per-pipeline.
...
- **[Group steps](/docs/pipelines/configure/step-types/group-step):** When a generator produces more than 30 steps, grouping by service or phase makes the build page easier to navigate.
