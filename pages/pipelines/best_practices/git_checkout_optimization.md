...
> 📘
> In addition to sparse checkout and Git mirrors, for checkout optimization you can also use the [Custom Checkout Buildkite Plugin](https://buildkite.com/resources/plugins/buildkite-plugins/custom-checkout-buildkite-plugin/) that allows configuring `--depth` flag for `git-clone` and `git-fetch` commands.
