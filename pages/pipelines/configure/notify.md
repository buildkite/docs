...
If a running job uses [early failure detection](/docs/pipelines/configure/early-failure-detection) and the promised exit status counts as a hard failure, `build.failing` notifications can be sent before that job exits.
