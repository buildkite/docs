...
&nbsp;&nbsp;- "src/"</code>
  ...
  <tr>
    ...
    <td>
      <p>Whether the agent should verify that the commit being built exists on the specified branch. Set to <code>strict</code> to fail the job when the agent determines the commit is not on the branch, or <code>off</code> to skip verification. If the agent cannot complete a <code>strict</code> check (for example, due to a shallow clone that cannot be deepened), it warns and continues. An empty value is invalid and causes job bootstrap to fail when the agent runs with <code>checkout-override-mode</code> set to <code>none</code>. Emitted as <a href="/docs/pipelines/configure/environment-variables#BUILDKITE_GIT_COMMIT_VERIFICATION"><code>BUILDKITE_GIT_COMMIT_VERIFICATION</code></a>. When omitted, the agent falls back to its own <code>--git-commit-verification</code> <a href="/docs/agent/self-hosted/configure#configuration-settings">configuration setting</a>. Buildkite agent v4 uses <code>strict</code> by default. Buildkite agent v3 does not verify commits unless you configure this setting.</p>
