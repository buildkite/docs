...
For a project that is already configured to use test collection, first set an OIDC policy for the suite you want to upload to, listing the pipelines you want to upload test results from. You can set the policy through the suite's **Settings** page in the Buildkite interface, or using the [suites REST API](/docs/apis/rest-api/test-engine/suites) with the `oidc_policy` field on create or update.
...
- iss: "https://agent.buildkite.com"
  ...
  scopes:
    - "read_suites"
    - "write_uploads"
    - "read_test_plan"  # Optional: only required when using the bktec CLI tool
    - "write_test_plan" # Optional: only required when using the bktec CLI tool
```
