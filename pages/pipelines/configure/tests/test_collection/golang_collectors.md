...
To use Buildkite Test Engine with your [Go](https://go.dev/) language projects, either use the [Tests Buildkite plugin](https://buildkite.com/resources/plugins/buildkite-plugins/tests-buildkite-plugin/) to run `go test` through [bktec](/docs/pipelines/configure/tests/bktec/installing-and-using-the-client), or use [gotestsum](https://github.com/gotestyourself/gotestsum) to generate JUnit XML files and [upload them](/docs/pipelines/configure/tests/test-collection/importing-junit-xml) to Buildkite Test Engine.
...
The following step uses the [Tests Buildkite plugin](https://buildkite.com/resources/plugins/buildkite-plugins/tests-buildkite-plugin/) to run `gotestsum` through bktec. The plugin downloads bktec, requests an OIDC token, ensures the test suite exists, and exports the environment variables that bktec expects, so the step's command only needs to invoke `bktec run`:
