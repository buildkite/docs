...
A workflow is configured with a _monitor_, which is a specialized type of observer to your [test suite](/docs/pipelines/configure/tests/test-suites). A monitor observes test [executions](/docs/pipelines/glossary#execution), and surfaces information and trends about the test's performance and reliability over time. Workflows are subject to a rate limit. See [Rate limit](/docs/pipelines/configure/tests/workflows#rate-limit) for more information.
...
In addition to the window, the transition counts that cause the [_alarm_ and _recover_ actions](/docs/pipelines/configure/tests/workflows/actions) to be triggered are configurable.
...
The monitor maintains a rolling window of recent [execution](/docs/pipelines/glossary#execution) durations for each test. The monitor calculates the aggregated duration across the window and compares it to the configured thresholds:

- When the aggregated duration is greater than or equal to the **alarm threshold**, an _alarm_ [action](/docs/pipelines/configure/tests/workflows/actions) is triggered.
...
Tag filters reduce the set of [execution](/docs/pipelines/glossary#execution) data that goes into a monitor, so that you can ignore lower relevancy data and produce better insights, or take different [actions](/docs/pipelines/configure/tests/workflows/actions) based on different types of test executions. This means that you can set up custom actions and monitors based on tag values, for example sending different notifications based on different team tag values, or using tags to segment the different types of test (e.g. feature, unit) and monitor on different thresholds.
...
If you haven't set up tags for test execution, see [Tags](/docs/pipelines/configure/tests/test-suites/tags) in the [Test suites](/docs/pipelines/configure/tests/test-suites) documentation for details.
