...
Workflows surface valuable qualitative information about the [tests](/docs/pipelines/glossary#test) in your [test suite](/docs/pipelines/configure/tests/test-suites), which can be difficult to surmise from raw execution data. A workflow defines a process which allows you to create custom mappings between _observations_ that Test Engine makes about your test suite, and the [_actions_](/docs/pipelines/glossary#action) you'd like to take from them. This means that observations about the health and performance of your tests (for example, test is flaky) can generate automatic actions (for example, label the test as flaky, send a notification).

Workflows are composed of a single [monitor](/docs/pipelines/configure/tests/workflows/monitors), optional [tag filters](/docs/pipelines/configure/tests/workflows/monitors#tag-filters), and a number of different [actions](/docs/pipelines/configure/tests/workflows/actions). The actionable insights generated by workflows can be used to improve your test suite, for example by [reducing the number of flaky tests](/docs/pipelines/reduce-flaky-tests).

## How they work

A single monitor watches over all the tests in your test suite (except for those excluded by filters) and generates individual _alarm_ and _recover_ events for each test, which then trigger the associated [alarm and recover actions](/docs/pipelines/configure/tests/workflows/actions).
...
- **Threshold changes take effect on the next test execution.** For example, if you adjust a [probabilistic flakiness](/docs/pipelines/configure/tests/workflows/monitors#probabilistic-flakiness) threshold or a [transition count](/docs/pipelines/configure/tests/workflows/monitors#transition-count) alarm value, the updated threshold is applied the next time test data is ingested for a matching test—not at a scheduled interval.
