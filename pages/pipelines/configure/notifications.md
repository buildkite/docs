...
* The _Slack_ notification service requires you to configure the channels, users, pipelines, branches and build states, as one or more notification services in Buildkite first. Doing this allows your pipelines to generate automated notifications, whenever the conditions in your notification services are met. However, you can also use the `notify` attribute in your `pipeline.yml` file for more fine grained control, by mentioning specific channels and users in these attributes, as long as the conditions in your notifications have still been met. If you mention any channels or users in a pipeline `notify` attribute for whom a Slack notification service has not yet been configured, the notification will not be sent. To avoid these issues, use the [Slack Workspace](/docs/pipelines/integrations/other/slack-workspace) notification service instead.
