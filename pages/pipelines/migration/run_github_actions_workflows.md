...
The trigger supports branch and tag pushes, plus same-repository pull requests with the `opened`, `reopened`, or `synchronize` activity by default. It supports `branches`, `branches-ignore`, `tags`, and `tags-ignore`. Pull requests can also use `types` to select activities explicitly. A workflow that explicitly lists `ready_for_review` in `types` also matches that activity, even though it isn't included by default. Pull request builds use the head branch and commit, while `GITHUB_WORKFLOW_REF` identifies `refs/pull/<N>/merge`. Fork pull requests, path filters, unsupported events or activities, and unsupported filter patterns fail closed and appear in **Recent Deliveries**.
