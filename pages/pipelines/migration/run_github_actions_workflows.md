...
> The plugin and runtime are under active development. Review the [`buildkite-gha` v0.29.0 compatibility guide](https://github.com/buildkite/buildkite-gha/blob/v0.29.0/docs/compatibility.md) before adding a workflow.
...
| `runners` | No | Fallback mappings from GitHub runner labels to Buildkite queues and optional Linux images. Agent API runner resolution takes precedence, except when it resolves a label through a heuristic fallback that a `runners` entry also maps, in which case the `runners` entry takes precedence. |
...
During upload, the runtime first asks the job-scoped Agent API to resolve each `runs-on` selector to a complete target: a queue, a platform, and, for Linux, an immutable image. Exact supported Ubuntu selectors (`ubuntu-latest`, `ubuntu-24.04`, and `ubuntu-22.04`) resolve to the hosted Linux queue with the matching Ubuntu image and no warning. Other selectors that look Linux-compatible, such as older `ubuntu-*` versions or custom self-hosted labels, also resolve to the hosted Linux queue using the latest Ubuntu image, but the job shows a warning annotation recommending an explicit runner mapping instead. Selectors for clearly incompatible operating systems, such as Windows or non-native macOS, or for non-AMD64 architectures, still fail closed with an unmapped labels error.

A result from the Agent API takes precedence over configured `runners` mappings and runtime presets, except when the result is a heuristic fallback for a label that a `runners` entry also maps, in which case the `runners` entry takes precedence. Add a `runners` entry to provide a fallback mapping from a GitHub runner label to a Buildkite queue and to avoid the fallback warning. Supported macOS labels use a native macOS fallback when available or require an explicit queue mapping.
