...
The trigger supports branch and tag pushes, plus same-repository pull requests. It supports `branches`, `branches-ignore`, `tags`, and `tags-ignore`. Without an explicit `types` filter, a pull request workflow triggers on the `opened`, `reopened`, and `synchronize` activity types. A pull request workflow can also use `types` to trigger explicitly on the `ready_for_review`, `converted_to_draft`, `labeled`, `unlabeled`, `review_request_removed`, `review_requested`, `locked`, `unlocked`, `milestoned`, `demilestoned`, `assigned`, `unassigned`, `enqueued`, `dequeued`, `closed`, `edited`, or `auto_merge_enabled` activity types. Pull request builds use the head branch and commit, while `GITHUB_WORKFLOW_REF` identifies `refs/pull/<N>/merge`. Fork pull requests, path filters, unsupported events or activity types, and unsupported filter patterns fail closed and appear in **Recent Deliveries**.
