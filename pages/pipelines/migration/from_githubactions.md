...
- Other Buildkite plugins you can use to override or customize the default checkout behavior are:
    ...
    * Sparse Checkout Buildkite plugin that performs a sparse checkout so only selected paths are fetched and checked out, reducing time and bandwidth on large repositories. See the plugin in the [Buildkite plugins directory](https://buildkite.com/resources/plugins/buildkite-plugins/sparse-checkout-buildkite-plugin/)
	* Custom Checkout Buildkite plugin that overrides the default agent checkout by setting a custom `refspec` and then doing a `git lfs pull`. See the plugin in the [Buildkite plugins directory](https://buildkite.com/resources/plugins/buildkite-plugins/custom-checkout-buildkite-plugin/).
