...
| `checkout`      | <span class="add-icon-plugin">Plugin (non-vendored)</span><br /><span class="add-icon-agent">Agent</span>                                                                                                                    | Overrides the default git checkout behavior, including the agent's built-in [commit verification](/docs/pipelines/configure/git-checkout#commit-verification). The checkout process is responsible for checking that the specified commit belongs to the specified branch. If you use a custom `checkout` hook, you are responsible for implementing this check in your hook. See [Hook exceptions](#job-lifecycle-hooks-hook-exceptions). |
