...
> 🚧
> Starting with version 0.29.0 of the controller, [unclustered agent tokens](/docs/agent/v3/self-hosted/tokens#working-with-unclustered-agent-tokens) are no longer supported. The Buildkite Agent Stack for Kubernetes requires a [Buildkite cluster](/docs/pipelines/security/clusters/manage) and an [agent token](/docs/agent/v3/self-hosted/tokens#create-a-token) for this cluster in order to process Buildkite jobs.
