...
You'd then add this key to the user's settings on [GitHub](/docs/agent/v3/self-hosted/github-ssh-keys), Bitbucket, GitLab, etc.
...
>📘
> Note that if you are using Elastic CI Stack for AWS, the following approach is redundant as the stack creates a [build secrets bucket](/docs/agent/v3/aws/elastic-ci-stack/ec2-linux-and-windows/security#s3-secrets-bucket) and allows you to specify an SSH key per pipeline as `/{pipeline-slug}/private_ssh_key`.

1. Add a pipeline-specific environment (for example, by using [Elastic CI Stack for AWS's build secrets bucket](/docs/agent/v3/aws/elastic-ci-stack/ec2-linux-and-windows/security#s3-secrets-bucket) or by having an Agent environment hook that switches on the repository URL or the pipeline slug):  
