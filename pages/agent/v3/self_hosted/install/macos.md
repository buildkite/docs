...
If you need the same configuration on each agent, either configure the `launchd` service to use the [`--spawn` flag](/docs/agent/v3/cli/reference/start#starting-an-agent-options) on the `buildkite-agent`, or the [`spawn` setting](/docs/agent/v3/self-hosted/configure#spawn) in the `buildkite-agent.cfg` file.
