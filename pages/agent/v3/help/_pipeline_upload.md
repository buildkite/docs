...
<tr id="apply-if-changed"><th><code>--apply-if-changed value</code> <a class="Docs__attribute__link" href="#apply-if-changed">#</a></th><td><p>When enabled, steps containing an `if_changed` key are evaluated against the git diff. If the `if_changed` glob pattern match no files changed in the build, the key is replaced with `skip`<br /><strong>Environment variable</strong>: <code>$BUILDKITE_AGENT_APPLY_SKIP_IF_UNCHANGED</code></p></td></tr>
