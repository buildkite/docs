...
All of the following claims (with the exception of the [`aud` claim](#aud), which is usually overridden by the<br/>[`--audience` option](#audience)) are automatically generated by the Buildkite Agent based on metadata from the pipeline job it is currently building.
