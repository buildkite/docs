...
Git mirrors work by mirroring the repository in a central location and making each checkout use a –reference clone of the mirror. It maintains a single bare git mirror for each repository on a host that is shared amongst multiple agents and pipelines. Checkouts reference the git mirror using `git clone --reference`, as do submodules.
