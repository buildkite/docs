...
By default, Buildkite runs jobs on the first available agent that matches the agent tags of these jobs, ordered by how recently that agent ran a successful job. This takes advantage of warm caches to guarantee the fastest run time possible. You can alter this by setting [priority](/docs/agent/v3/prioritization) for some or all of your agents.
