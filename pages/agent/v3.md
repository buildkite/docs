...
>🛠 Experimental feature
> To use flock file locks, set <code>experiment="kubernetes-exec"</code> in your <a href="/docs/agent/v3/configuration#experiment"> agent configuration</a>.
...
The API is exposed via a Unix Domain Socket, whose path is exposed to running jobs with the `BUILDKITE_AGENT_JOB_API_SOCKET` environment variable, and authenticated with a token exposed using the `BUILDKITE_AGENT_JOB_API_TOKEN` environment variable, using the `Bearer` HTTP Authorization scheme.
...
See [the agent repo](https://github.com/buildkite/agent/blob/main/jobapi/payloads.go) for the full API request/response definitions.
...
>🛠 Experimental feature
> To use flock file locks, set <code>experiment="job-api"</code> in your <a href="/docs/agent/v3/configuration#experiment"> agent configuration</a>.
