...
### Git mirrors
Most users are familiar with git clone which is primarily used to produce a copy of a repository a user can then use to make changes or updates to the original repository using the underlying operations hidden in a secret .git directory. Users may also have used git clone –bare which exposes all the hidden underlying Git operations at the expense of not checking out the main branch for the users in the root of the directory 
Git mirror is similar to git clone –bare with the main difference being that it also clones all the extra elements you normally would not get with git clone or git clone –bare. Git Mirror actually provides all the branches associated with that repository which is very important in a real scenario as the agent does not know which branch it needs to build. This is very useful especially if you need to save time cloning a specific branch as git mirror makes a clone of everything in the remote repository (practically a complete mirror of the remote repository) 
...
Speed up cloning
Reduce disk usage 
Faster build operation
Back up for repositories 
