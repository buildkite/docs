...
### Git mirrors
Firstly, most users are familiar with <code> git clone </code> which is primarily used to produce a copy of a repository users can utilize to make changes or updates to the original repository. This is done using the underlying operations hidden in a secret <code>.git</code> directory. Users may also have used <code> git clone –-bare</code> which exposes all the hidden underlying Git operations at the expense of not checking out the main branch for the users in the root of the directory.
