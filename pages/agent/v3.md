...
Promoted in [v3.48.0](https://github.com/buildkite/agent/releases/tag/v3.48.0). It's the default behavior, so there's no configuration required to use it. Because the old and new lock systems do not interact, we _strongly_ recommend not running different versions of the agent on the same host.
