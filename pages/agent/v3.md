...
You can install the agent on a wide variety of platforms, see the [installation instructions](/docs/agent/v3/self-hosted/installing) for a full list and for information on how to get started.
...
Git mirrors is no longer an experimental feature. Promoted in [v3.47.0](https://github.com/buildkite/agent/releases/tag/v3.47.0). You can use Git mirrors by setting the `--git-mirrors-path` flag. See the [Git mirrors](/docs/agent/v3/git-mirrors) to learn more about how Git mirrors work with agents running in your own self-hosted infrastructure.
