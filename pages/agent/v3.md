...
By default, Buildkite Pipeline runs jobs on the first available agent associated with the relevant [queue](/docs/agent/v3/queues) that the pipeline's [cluster](/docs/pipelines/security/clusters) is associated with. Agents within that queue are ordered by how recently that agent completed a job. This takes advantage of warm caches to guarantee the fastest run time possible.
