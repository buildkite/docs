...
<%= render "agent/cli/help/oidc_request_token" %>
...
For example, this command adds the Buildkite cluster's UUID value as a claim to the OIDC token:

```sh
$ buildkite-agent oidc request-token ... --claim "cluster_id"
```

This command adds both the cluster's UUID and queue's UUID values in their own additional claims to the OIDC token:

```sh
$ buildkite-agent oidc request-token ... --claim "cluster_id,queue_id"
```
