...
> 📘 Flock file locks
> The Buildkite agent also has an internal `flock` file locking mechanism, which is an automatic feature that's unrelated to the locking feature provided by these agent `lock` commands. The `flock` mechanism is used for Git mirror handling, and these locks are automatically released when the process is completed, including when the process terminates abnormally, for example, when an agent is not cleanly shut down.
