...
Automatic ssh-keyscan can be disabled by setting [`no-ssh-keyscan`](/docs/agent/self-hosted/configure#no-ssh-keyscan):
...
If you still want to use plugins, you can check out a tool for [signing pipelines](/docs/agent/self-hosted/security#sign-your-pipelines).
...
Command line evaluation can be disabled by setting [`no-command-eval`](/docs/agent/self-hosted/configure#no-command-eval):
...
To restrict agent connection by IP address, set the [**Allowed IP Addresses** attribute](/docs/pipelines/security/clusters/manage#restrict-an-agent-tokens-access-by-ip-address). This restricts agent registration to those IPs, and any existing agents outside the allowed IP ranges will be forcefully disconnected.
