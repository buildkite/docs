...
Buildkite hosted agents can access private repositories in GitHub natively, by authorizing Buildkite to access these GitHub repositories. To access private repositories from another provider, the [git-ssh-checkout](https://github.com/buildkite-plugins/git-ssh-checkout-buildkite-plugin) plugin is available to provide this capability.
...
1. Add the [git-ssh-checkout](https://github.com/buildkite-plugins/git-ssh-checkout-buildkite-plugin) plugin to the initial pipeline steps, and any further steps within the uploaded pipeline.
