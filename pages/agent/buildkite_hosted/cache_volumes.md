...
> [Buildkite Cache](/docs/pipelines/configure/cache) is a separate, key-based caching feature that works on both Buildkite hosted agents and self-hosted agents. Unlike a volume, it restores the specific entry a job asks for, and its cache registries support access policies. Buildkite Cache is available in public preview and is enabled for all Buildkite organizations. You don't need to request access. To compare the two features, see [Buildkite Cache compared with cache volumes](/docs/pipelines/best-practices/caching#choosing-a-caching-approach-buildkite-cache-compared-with-cache-volumes).
