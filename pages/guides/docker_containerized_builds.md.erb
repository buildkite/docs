...
For Docker-based builds we recommend using [Docker Compose](https://docs.docker.com/compose/) as it allows each pipeline to define it’s own `docker-compose.yml` with dependent containers and environment variables to be passed through.
...
To allow buildkite-agent to use the Docker client you’ll need to ensure its user has the necessary permissions. For most platforms this means adding the `buildkite-agent` user to your system’s `docker` group, and then restarting the Docker daemon. See your platform’s [Docker installation instructions](https://docs.docker.com/installation/) for more details.
