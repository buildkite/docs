...
> When you pass more than one value in `agentQueryRules`, a job must match all of them (the rules are combined with AND, not OR). For example, `agentQueryRules: ["queue=test", "tag=value"]` returns only jobs whose agent query rules include both `queue=test` and `tag=value`.
