...
> The timestamp in the `X-Buildkite-Signature` header is the time the webhook HTTP request was dispatched, not the time the underlying event occurred. Its purpose is replay-attack prevention—the timestamp is included in the HMAC so stale webhooks can be rejected.
