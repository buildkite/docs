...
Save the returned `endpoint_url` securely before leaving the creation response. It contains a one-time plaintext credential and isn't returned by subsequent reads. Webhook provisioning returns a `webhook_creation` result in the response. Check its `status`: `201 Created` can still include `webhook_creation.status: "failed"`, because the trigger itself was created successfully.
