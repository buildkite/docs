...
The internal job API is one that's exposed locally/internally within the agent for the currently running job, which allows you to query and mutate the state of this job through environment variables. This lets you write scripts, hooks, and plugins in languages other than Bash, using them to interact with the agent.
