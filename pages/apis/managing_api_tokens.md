...
Removing access from a token will send a notification email to the token's owner, and they will not be able to re-add your organization to the token's scope.
...
If an organization has revoked a token, it cannot be re-added to the token. The token owner would have to create a new token with access to your organization.
