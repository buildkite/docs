...
A token doesn't grant access to an organization with no active plan. The API omits those organizations, and organization-scoped requests return `404` with `{"message":"No organization found"}`. See [Organizations API](/docs/apis/rest-api/organizations).
