...
Read-only access to Buildkite is available by connecting to the remote MCP server at the following URL:

```
https://mcp.buildkite.com/mcp/readonly
```

This will reduce the requested access permissions to read-only scopes, and only expose read-only tools.
