...
To omit a table of contents, use `{:notoc}`. Typically `{:notoc}` is for pages where the text immediately following \#-level is the body of the page or where there's only one \##\-level heading, which looks lonely in a table of contents.

A page can have one and only one of `{:toc}` or `{:notoc}`.
