...
- Plugin hooks - these are provided by any [plugins](/docs/plugins) you've
  ...
  them. Plugins can be _vendored_ (if they are already present in the repository,
  and included using a relative path) or _non-vendored_ (when they are included
...
| `checkout`      | <span class="add-icon-plugin">Plugin (non-vendored)</span><br /><span class="add-icon-agent">Agent</span>                                                                                                                      | Overrides the default git checkout behavior.<br />_Note:_ As of Agent v3.15.0, if multiple checkout hooks are found, only the first will be run. |
...
Job lifecycle hooks are copied to `$TMPDIR` directory and _sourced_ by the agent's default shell.
